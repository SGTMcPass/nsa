...
def normalize_href(href: str, base: Path, docs_root: Path) -> Path | None:
    ...
    # Add .md suffix if missing
    if target.suffix == '':
        target = target.with_suffix('.md')
    # Ensure it's under docs_root
    try:
        if target.is_file() and target.resolve().is_relative_to(Path(docs_root).resolve()):
            return target
...
def classify(path: Path, cfg: Dict) -> str:
    ...
    if any(tok in name for tok in ('howto', 'tutorial', 'guide')):
        return 'howto'
    if any(tok in name for tok in ('overview', 'introduction', 'architecture', 'concept')):
        return 'concept'
