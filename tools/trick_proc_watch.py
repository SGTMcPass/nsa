...
def is_target(proc: psutil.Process) -> bool:
    try:
        ...
        return False
    return name.startswith("S_main") or re.fullmatch(
        r"python(\d+(\.\d+)*)?(\.exe)?", name
    )
...
def watch(user: str, interval: float, cpu_idle: float, samples: int):
    ...
    header = (
        f"{BOLD}{'UID':<8} {'PID':>6} {'PPID':>6} {'CPU%':>5} {'MEM%':>5} "
    ...
    while True:
        ...
        for proc in psutil.process_iter(
            attrs=["pid", "ppid", "name", "username", "create_time", "cmdline"]
        ):
            try:
                ...
                print(
                    f"{user:<8} {proc.pid:>6} {proc.ppid():>6} {cpu_s} {mem_s} "
...
def main(argv: List[str] | None = None):
    ap = argparse.ArgumentParser(
        description="Watch Trick & Python procs with stall flag"
    )
